...
var trigger = past
    |join(data)
        .as('past', 'current')
    |eval (lambda: float("current.value" - "past.value")@TREND_TYPE@)
        .keep()
        .as('value')
    @resInjector()
        .alertId(ID_ALERT)
    ...
    |alert()
        .info(lambda: if("check_info", if(float("value") @DIRECTION0@ 0, float("value") @DIRECTION@ if("mon_exc" == 0, TH_INFO_DEF, if("mon_exc" == 1, TH_INFO_EX1, if("mon_exc" == 2, TH_INFO_EX2, 0.0))),FALSE),FALSE))
        .warn(lambda: if("check_warn", if(float("value") @DIRECTION0@ 0, float("value") @DIRECTION@ if("mon_exc" == 0, TH_WARN_DEF, if("mon_exc" == 1, TH_WARN_EX1, if("mon_exc" == 2, TH_WARN_EX2, 0.0))),FALSE),FALSE))
        .crit(lambda: if("check_crit", if(float("value") @DIRECTION0@ 0, float("value") @DIRECTION@ if("mon_exc" == 0, TH_CRIT_DEF, if("mon_exc" == 1, TH_CRIT_EX1, if("mon_exc" == 2, TH_CRIT_EX2, 0.0))),FALSE),FALSE))
